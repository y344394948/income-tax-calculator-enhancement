...
IncomeTaxQuickLookupItem = namedtuple( #设置特定的属性名，用于访问列表
...
def main():
    for item in sys.argv[1:]: #利用for循环遍历传入的多个收入，进行切片处理
        employee_id, income_string = item.split(':') #进行分割
        try:
            income = int(income_string)
        except ValueError:
            print('Parameter Error')
        _, remain = calc_income_tax_and_remain(income) #_ 当生成的结果需要丢弃时可使用
